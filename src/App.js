...
export function App() {
    ...
    async function fetchData() {
      try {
        ...
        const cartItemsResponse = await axios.get(
          'https://63fce95c859df29986c75869.mockapi.io/cart',
        );
        const likedItemsResponse = await axios.get(
          'https://64248fef7ac292e3cfed5b72.mockapi.io/Liked_items',
        );
  ...
  async function makeOrder() {
    try {
      ...
      const { data } = await axios.post('https://64248fef7ac292e3cfed5b72.mockapi.io/Orders', {
        items: cartItems,
      });
      setOrderId(data.id);
      setIsOrdered(true);
      ...
      <nav className={styles.navigation}>
        <div className={styles.search}>
          <img
            ...
            src="/img/icons/magnifying-glass.svg"></img>
          <input onChange={onChangeSearchInput} type="text" placeholder="SEARCH"></input>
        </div>
        <Link to="/orders">
          <div className={styles.icons}>
            <img alt="user" src="/img/icons/user.svg"></img>
          </div>
        </Link>
        <Link to="/liked">
          <div className={styles.icons}>
            <img alt="liked-items" src="img/icons/heart-fill.svg"></img>
          </div>
        </Link>
        <div className={styles.icons} onClick={changeCart}>
          <img alt="bag" src="img/icons/bag-outline.svg"></img>
        </div>
      </nav>
